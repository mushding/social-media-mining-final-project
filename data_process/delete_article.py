...
def articleFilter(article_df):
    # 把前 50% 少的留言數的文章去掉
    threshold = article_df['comments_count'].quantile([0.5]).iloc[0]
    # Refer to the MovieLens dataset
    threshold = 20
    print("Delete the articles less than {:>3} comments.".format(int(threshold)))
...
def combine(df, article_df):
    ...
    for _ in range(len(df)):
        if article_df_index > len(article_df)-1:
            break
...
if __name__ == "__main__":
    ...
    print("len article_df: ", len(article_df))
